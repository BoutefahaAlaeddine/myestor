  ...
  <form action="" method="post" class="pad-10">
    <div class="row d-flex">
      <div class="inputs flex pad-5 p-rel">
        ...
        <input name="phoneNumber" type="text" placeholder="<?= $text_label_phoneNumber ?>" value="<?= $user->PhoneNumber ?>">
      ...
      <div class="inputs flex pad-5 p-rel">
        <select name="GroupId" id="">
          ...
          <?php foreach ($groups as $group) :  ?>
            <option value="<?= $group->GroupId ?>" <?= $user->GroupId == $group->GroupId ? "selected" : "" ?>><?= $group->GroupName ?></option>
          <?php endforeach;  ?>
        </select>
      </div>
    </div>
